...
contract PiTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SSax }
    ...
    address public sSax;
    uint public sSaxQueue; // Delays change to sSax address
    ...
    /**
        @notice allow approved address to deposit an asset for Sax
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint Sax needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( Sax ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn Sax for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        ISaxERC20( Sax ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sSax ).balanceOf( msg.sender ); // Can only borrow against sSax held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with Sax
        @param _amount uint
     */
    function repayDebtWithSax( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        ISaxERC20( Sax ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, Sax, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( Sax ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( Sax ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns Sax valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match Sax decimals
            value_ = _amount.mul( 10 ** IERC20( Sax ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SSax ) { // 9
            sSaxQueue = block.number.add( blocksNeededForQueue );
        } else return false;
